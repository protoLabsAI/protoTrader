...
| Finance domain layer | `plugins/prototrader-finance` | The fork's value-add as **one full-bundle plugin** (the plugin-devkit pattern): 13 finance tools (market data, backtest engine, factor IC scoring, behavioral journal, **gated paper broker**), the 3-subagent research desk, the `quant-desk` / `investment-committee` workflows + finance skills (bundled, auto-discovered), and a **Quant Desk dashboard console view** (ADR 0026). Installable into any protoAgent via `plugin install <git-url>` (ADR 0027). See [Finance tools](./docs/reference/finance-tools.md) + [The desk](./docs/guides/the-desk.md) |
