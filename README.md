...
| Finance domain layer | `plugins/{finance-data,backtest,factors,behavioral,broker,finance-desk}` | The fork's value-add: 15 finance tools (market data, backtest engine, factor IC scoring, behavioral journal, **gated paper broker**), the 3-subagent research desk, and the `quant-desk` / `investment-committee` workflows. See [Finance tools](./docs/reference/finance-tools.md) + [The desk](./docs/guides/the-desk.md) |
